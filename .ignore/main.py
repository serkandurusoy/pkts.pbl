...
def doConnect():
    ...
    if not sta_if.isconnected():
        ...
        #sta_if.connect('I.Cat.HQ', '1455268989')
        ...
        #sta_if.connect('Laperokas','laperokas06');
...
def startUserMode():
    ...
    while True:
        ...
        if buttonStates is '1101':
            ...
            talkToServer(moduleID + '.Button.3.Count.' + str(button3Count))
            pass
...
def talkToServer(message):
    ...
    address = socket.getaddrinfo('10.10.10.104',9000)[0][-1]
    mySocket = socket.socket()
    mySocket.connect(address)
    mySocket.send(bytes(message, 'utf8'))
    print(moduleID + ' SND [' + message + ']')
    while True:
